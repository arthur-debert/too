...
// IDMResult represents the result of a move operation using pure IDM data structures.
...
// ExecuteIDM moves a todo from one parent to another using the pure IDM manager.
func ExecuteIDM(sourcePath string, destParentPath string, opts Options) (*IDMResult, error) {
...
func ConvertIDMResultToResult(idmResult *IDMResult) *Result {
	result := &Result{
		...
		OldPath: idmResult.OldPath,
		NewPath: idmResult.NewPath,
	}

	if idmResult.OldParent != nil {
		result.OldParent = convertIDMTodoToTodo(idmResult.OldParent)
	}

	if idmResult.NewParent != nil {
		result.NewParent = convertIDMTodoToTodo(idmResult.NewParent)
	}
