...
func TestIntegration_CompleteReopenList(t *testing.T) {
	t.Run("reopening an item correctly places it back in the active list", func(t *testing.T) {
		...
		_, err := tdh.Complete("2", tdh.CompleteOptions{CollectionPath: path})
		testutil.AssertNoError(t, err)

		// 2. Verify the active list is correct
		listResult1, err := tdh.List(tdh.ListOptions{CollectionPath: path})
		testutil.AssertNoError(t, err)
		...
		_, err = tdh.Reopen(archivedRef, tdh.ReopenOptions{CollectionPath: path})
		testutil.AssertNoError(t, err)

		// 5. List the active items again
		listResult2, err := tdh.List(tdh.ListOptions{CollectionPath: path})
		testutil.AssertNoError(t, err)
